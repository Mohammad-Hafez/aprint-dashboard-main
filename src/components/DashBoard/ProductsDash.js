...
import {AddHeader,AddOffer , UpdateOffers , AddNestedOfOptions,AddOptions,AddProduct,AddSection,DeleteHeader,DeleteOptions, DeleteProduct,DeleteSection,UPdateProduct, UpdateFooter,UpdateOptions,getOneProduct,getProducts, DeleteOffer,} from "../../store/ProductsSlice";
...
const ProductsDash = () => {
  ...
  useEffect(() => {if (!Products) {dispatch(getProducts())}},[Products, dispatch]);
  useEffect(() => { if (!proCateg) {dispatch(getProductsCategories(2))}}, [dispatch, proCateg]);
        ...
        <div className="flex flex-column align-items-center" style={{ flex: "1" }} >
          ...
          <div className="flex gap-2">
            <Button onClick={(e) => {
              ...
              type="button" label="Confirm" className="p-button-success w-6rem" />
  ...
  const renderHeader2 = () => {
    return (
      <div className={`flex justify-content-between flex-wrap ${styles.tabel_header}`}>
        <button className={styles.addBTN} onClick={() => { setVisible(true); setSavetype("Save");}}>
          <span className="d-flex align-items-center"><Icon icon={plus}/> Add Products</span>
        </button>
      </div>
    );
  };
  ...
  const StatusBody = (rowData) => {
    return (
      <div className={styles.TB_Content}>
        <button className={`${styles.TabelButton} ${styles.Edite}`} onClick={() => {  // Edit Btn 
            setSavetype("update");
            setVisible(true);
            ...
            setMaxQuantity(rowData?.max_q);
            setMinQuantity(rowData?.min_q);
          }}
        >
          <AiFillEdit />
        </button>
  ...
  const [limit, setMetaLimit] = useState(0); // You can initialize it with a default value
  ...
  const Create = async () => {
    ...
    if (Savetype === "Save") {
      if ( name.length > 0 && description0.length > 0 && upImage && category_id &&
        price && max_width &&  min_width && max_height &&  min_height && MaxQuantity && MinQuantity &&
        def_width && meta_title.length > 0 && meta_description.length > 0 && limit && def_height
      ) {
  ...
  const [OfferDimension, setOfferDimension] = useState("");
  const [OfferDiscount, setOfferDiscount] = useState("");
  const [OffersTabel, setOffersTabel] = useState([]);
  ...
  const offersBody = (rowData) => {
    return (
      <div className={styles.TB_Content} style={{ justifyContent: "center" }}>
        <button className={`${styles.TabelButton} ${styles.Edite}`}
          onClick={() => {
            dispatch(getOneProduct(rowData.id)).unwrap().then((res) => {
                if (res.data.offers){
                  setOffersTabel([...res.data.offers]);
                  setProductId(res.data.id);
                  setOfferDialog(true);  
                }
              });
  ...
  const CreateOffer = () => {
    if (OfferDimension.length <= 0 || OfferDiscount.length <= 0) {
      showError("Please fill All Data");
    } else {
      const data = {
  ...
  const OfferStatusBody = (rowData) => {
    return (
      <div className={styles.TB_Content} style={{ justifyContent: "center" }}>
        <button className={`${styles.TabelButton} ${styles.Edite}`} onClick={() => {
          setOfferDialog(true);
          setSelectOffer(rowData);
          setOfferDimension(rowData.dimension);
          setOfferDiscount(rowData.discount);
          setSelectOfferId(rowData.id);
          }}>
        <CiEdit/>
        </button>
        <button className={`${styles.TabelButton} ${styles.Cancel}`}
          onClick={() => {
            dispatch(DeleteOffer(rowData.id))
              .unwrap()
              .then((res) => {
                if (res.success) {
                  showSuccess();
  ...
  return (<>
      ...
      <div className={styles.Tabel}>
        <DataTable paginator selectionMode="single" value={Products} className={`${styles.dataTabel}`} dataKey="id" scrollable scrollHeight="100vh" filterDisplay="row"  responsiveLayout="scroll" tableStyle={{ minWidth: "50rem" }} header={header2} rows={10} >
          <Column field="title" header=" Name " style={{ maxWidth: "7rem" }} />
          <Column field="price" header=" Price " style={{ maxWidth: "5rem" }} />
          <Column field={"category.name"} header=" Category" style={{ maxWidth: "7rem" }}/>
          <Column header=" Images" body={ImagesBody} style={{ maxWidth: "7rem" }} />
          <Column body={HeaderBody} header="Header " style={{ maxWidth: "7rem" }} />
          <Column body={FooterBody} header="Footer" style={{ maxWidth: "7rem" }} />
          <Column body={offersBody} header="Offers" style={{ maxWidth: "7rem" }} />
          <Column body={SectionsBody} header="Sections & Options" style={{ maxWidth: "7rem" }} />
          <Column header=" Status " body={StatusBody} style={{ maxWidth: "7rem" }} />
        </DataTable>
        <Dialog visible={visible} style={{ maxWidth: "90vw" }} onHide={() => { ClearData()}} >
          ...
          <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${name.length <= 0 ? "p-invalid" : ""}`} id="username" value={name} onChange={(e) => setName(e.target.value)} />
                <label htmlFor="username"> Name</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber minFractionDigits={2} maxFractionDigits={5} className={`${!price ? "p-invalid" : ""}`} id="Price" value={price} onValueChange={(e) => setPrice(e.value)} />
                <label htmlFor="Price"> Price</label>
              </span>
            </div>
            {proCateg && ( <div className="col-12 md:col-4  mt-5">
                <span className="p-float-label">
                  <Dropdown id="Category" value={category_id} onChange={(e) => { setCategory_id(e.value)}} options={proCateg} optionLabel="name" placeholder="Select Category" className={`${!category_id ? "p-invalid" : ""} w-full`} />
            ...
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber minFractionDigits={2} maxFractionDigits={5} className={`${!def_width ? "p-invalid" : ""}`} id="def_width" value={def_width} onValueChange={(e) => Setdef_width(e.value)} />
                <label htmlFor="def_width"> Default Width</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber  minFractionDigits={2} maxFractionDigits={5} className={`${!def_height ? "p-invalid" : ""}`} id="def_height" value={def_height}  onValueChange={(e) => Setdef_height(e.value)} />
                <label htmlFor="def_height"> Default Height</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber minFractionDigits={2} maxFractionDigits={5} className={`${!min_width ? "p-invalid" : ""}`} id="min_width" value={min_width} onValueChange={(e) => Setmin_width(e.value)} />
                <label htmlFor="min_width"> Min Width</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber minFractionDigits={2} maxFractionDigits={5} className={`${!min_height ? "p-invalid" : ""}`} id="min_height" value={min_height} onValueChange={(e) => Setmin_height(e.value)}/>
                <label htmlFor="min_height"> Min Height</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber  minFractionDigits={2} maxFractionDigits={5} className={`${!max_width ? "p-invalid" : ""}`} id="max_width" value={max_width} onValueChange={(e) => Setmax_width(e.value)} />
                <label htmlFor="max_width"> Max Width</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber minFractionDigits={2} maxFractionDigits={5} className={`${!max_height ? "p-invalid" : ""}`} id="max_height" value={max_height} onValueChange={(e) => Setmax_height(e.value)} />
                <label htmlFor="max_height"> Max Height</label>
              </span>
            </div>
            {/* *NOTE - Min & Max Quantity */}
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber className={`${!MaxQuantity ? "p-invalid" : ""}`} id="MaxQuantity" value={MaxQuantity} onValueChange={(e) => setMaxQuantity(e.value)} />
                <label htmlFor="MaxQuantity"> Max Quantity</label>
              </span>
            </div>
            <div className="col-12 md:col-4 mt-5">
              <span className="p-float-label w-full">
                <InputNumber className={`${!MinQuantity ? "p-invalid" : ""}`} id="MinQuantity" value={MinQuantity} onValueChange={(e) => setMinQuantity(e.value)} />
                <label htmlFor="MinQuantity"> Min Quantity</label>
              </span>
            </div>
            {/* *STUB - End Min& Max Quantity */}
            <div className="col-12 md:col-6 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${meta_title.length <= 0 ? "p-invalid" : ""}`} id="usermeta_title" value={meta_title} onChange={(e) => setmeta_title(e.target.value)} />
                <label htmlFor="meta_title"> Meta Title</label>
              </span>
            </div>
            <div className="col-12 md:col-6 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${meta_description.length <= 0 ? "p-invalid" : ""}`}id="meta_description"value={meta_description}onChange={(e) => setmeta_description(e.target.value)}/>
                <label htmlFor="meta_description"> Meta Descriprion</label>
              </span>
            </div>
            <div className="col-12 md:col-12 mt-5">
              <span className="p-float-label w-full">
                <InputNumber className={`${meta_description.length <= 0 ? "p-invalid" : ""}`} id="limit"value={limit}onValueChange={(e) => setMetaLimit(e.value)}/>
                <label htmlFor="limit"> Limit </label>
              </span>
            </div>
            <div className="col-12  mt-5">
              <span className="p-float-label">
                <InputTextarea id="decription"value={description0}className={`${description0.length <= 0 ? "p-invalid" : ""}`}onChange={(e) => setDescription0(e.target.value)}rows={5}cols={30}/>
                <label htmlFor="decription"> Description </label>
              </span>
            </div>
            {showmainImage &&
              !upImage && showmainImage.map((ele) => (
            ...
            <div className="col-12  mt-5">
              <div className={styles.change_store_image} style={{ borderColor: !upImage ? "red" : "#666d92" }} >
                <input type="file" style={{ display: "none" }}id="storeImage"name="storeImage"accept="image/*" multiple={true} onChange={(e) => {setUpImages(e.target.files)}}/>
          ...
          <div className="flex justify-content-center align-items-center">
            <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => Create()}>
              Save
            </button>
          </div>
        </Dialog>
        <Dialog visible={HeaderDialog}style={{ maxWidth: "90vw" }}onHide={() => {ClearHeaders()}}>
          <h1 className="main-two">Add Headers</h1>
          <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
            <div className="col-12 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${headerTitle.length <= 0 ? "p-invalid" : ""}`}id="headerTitle" value={headerTitle}onChange={(e) => setHeaderTitle(e.target.value)}/>
                <label htmlFor="headerTitle"> Title</label>
              </span>
            </div>
            <div className="col-12  mt-5">
              <div className="p-float-label w-full p-fluid">
                <InputText id="tags"value={headerLines}onChange={(e) => setHeaderLines(e.target.value)}/>
            ...
            <div className="col-12  mt-5">
              <span className="p-float-label">
                <InputTextarea id="Headerdescription" value={Headerdescription} className={`${ Headerdescription.length <= 0 ? "p-invalid" : "" }`}onChange={(e) => setHeaderDescription(e.target.value)}rows={5}cols={30} />
                <label htmlFor="Headerdescription"> Description </label>
              </span>
            </div>
          </form>
          <div className="flex justify-content-center align-items-center">
            <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => Createheader()} >
              Save
            </button>
          </div>

          <DataTable paginator selectionMode="single" value={HeaderTabel} className={`${styles.dataTabel}`} dataKey="id" scrollable scrollHeight="100vh" filterDisplay="row" responsiveLayout="scroll"
            tableStyle={{ minWidth: "50rem"}} rows={10} >
            <Column field="title" header=" Name " style={{ maxWidth: "7rem" }} />
            <Column field="description" header=" Description " style={{ maxWidth: "7rem" }} />
            <Column body={LinesBody} header=" Liens " style={{ maxWidth: "7rem" }} />
            <Column header=" Status " body={HeaderStatusBody} style={{ maxWidth: "7rem" }} />
          </DataTable>
        </Dialog>
        <Dialog visible={FooterDialog}style={{ maxWidth: "90vw" }}onHide={() =>{ClearFooters()}}>
          <h1 className="main-two">Product Footer</h1>
          <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
            <div className="col-12">
              <ReactQuill style={{background: "white",height: "500px"}}theme="snow"value={Footerdescription}onChange={setFooterDescription}modules={modules}/>
            </div>
          </form>
          <div className="flex justify-content-center align-items-center mt-5">
            <button className={`${styles.addBTN} mt-5 text-center`}onClick={() => CreateFooter()}>
              Save
            </button>
          </div>
        </Dialog>
        <Dialog visible={OfferDialog}style={{ maxWidth: "90vw" }}onHide={() => {ClearOffers()}}>
        <h1 className="main-two">
          {SelectOffer ? `Edit Offer` :`Add Offers `}
          </h1>
          <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
            <div className="col-12 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${OfferDimension?.length <= 0 ? "p-invalid" : ""}`} id="OfferDimension" value={OfferDimension}onChange={(e) => setOfferDimension(e.target.value)}/>
                <label htmlFor="OfferDimension"> Dimensions</label>
              </span>
            </div>
            <div className="col-12 mt-5">
              <div className="p-float-label w-full p-fluid">
          ...
          <div className="flex justify-content-center align-items-center">
            {SelectOffer ? <>
            <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => putUpdateOffer()} >
              Edit
            </button>
            </>:<>
            <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => CreateOffer()} >
              Save
            </button>            
            </>}
          </div>
        ...
        <Dialog visible={SectionDialog}style={{ maxWidth: "90vw" }}onHide={() =>{ClearSections()}}>
          <h1 className="main-two">Add Section</h1>
          <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
            <div className="col-12 mt-5">
              <span className="p-float-label w-full">
                <InputText className={`${SectionTitle.length <= 0 ? "p-invalid" : ""}`}id="SectionTitle"value={SectionTitle}onChange={(e) => setSectionTitle(e.target.value)}/>
                <label htmlFor="SectionTitle"> Title</label>
              </span>
            </div>
            <div className="col-12  mt-5">
              <span className="p-float-label">
                <InputText keyfilter={"int"} id="SectionOrder"value={SectionOrder}className={`${SectionOrder.length <= 0 ? "p-invalid" : ""}`}onChange={(e) => setSectionOrder(e.target.value)}/>
                <label htmlFor="SectionOrder"> Section Order </label>
              </span>
            </div>
          </form>
          <div className="flex justify-content-center align-items-center">
            <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => CreateSection()} >
              Save
            </button>
          </div>
          ...
          <DataTable paginator selectionMode="single"value={SectionTabel}className={`${styles.dataTabel}`}dataKey="id" scrollablescrollheight="100vh" filterDisplay="row"responsiveLayout="scroll"tableStyle={{ minWidth: "50rem" }}rows={10}>
            ...
            <Column header=" Status " body={SectionStatusBody} style={{ maxWidth: "7rem" }} />
          </DataTable>
        </Dialog>
          ...
          {AddNestedOfOption ? (<>
              {OptionsNum % 2 === 0 ? (<>
                  ...
                  <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`}>
                    <div className="col-12 md:col-4 mt-5">
                      <span className="p-float-label w-full">
                        <InputText className={`${ OptionTitle.length <= 0 ? "p-invalid" : "" }`} id="OptionTitle" value={OptionTitle} onChange={(e) => setOptionTitle(e.target.value)} />
                    ...
                    <div className="col-12 md:col-4  mt-5">
                      <span className="p-float-label">
                        <Dropdown id="Option_Price_Type" value={option_type} onChange={(e) => {setOption_type(e.value)}}
                          options={OptionsType}optionLabel="name"placeholder="Select Price Type"className={`${!option_type ? "p-invalid" : ""} w-full`}/>
                        <label htmlFor="Price Type">Price Type </label>
                      </span>
                    </div>
                    <div className="col-12  mt-5">
                      <div className={styles.change_store_image}style={{ borderColor: !OptionImage ? "red" : "#666d92"}}>
                        <input type="file"style={{ display: "none" }} id="storeImage" name="storeImage" accept="image/*" onChange={(e)=>{setOptionImage(e.target.files[0])}} />
                        <label htmlFor="storeImage">
                          {" "} <span className="icon-contact_mail"></span>
                        </label>
                        <label htmlFor="storeImage">
              ...
              <div className="flex justify-content-center align-items-center">
                <button className={`${styles.addBTN} mt-5 text-center`} onClick={(id) => AddSubOptions(id)} >
                  Add Sub Option
                </button>
              </div>
            </>
            ...
            <>
              ...
              <form className={`grid justify-content-center align-items-center ${styles.Dialog_Div}`} >
                ...
                <div className="col-12 md:col-4  mt-5">
                  <span className="p-float-label">
                    <Dropdown id="Option_Price_Type" value={option_type} onChange={(e) => { setOption_type(e.value)}} options={OptionsType} optionLabel="name"  placeholder="Select Price Type" className={`${!option_type ? "p-invalid" : ""} w-full`} />
                    <label htmlFor="Price Type">Price Type </label>
                  </span>
                </div>
                <div className="col-12   mt-5">
                  <span className="p-float-label">
                    <InputText id="OptionDescription" value={OptionDescription} onChange={(e) => setOptionDescription(e.target.value)} rows={5} cols={30} />
            ...
            <>
              ...
              <div className="flex justify-content-center align-items-center">
                <button className={`${styles.addBTN} mt-5 text-center`} onClick={() => CreateOption()} >
                  Save
                </button>
              </div>
            </>
